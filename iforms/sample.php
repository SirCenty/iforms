                ...
                <table  style="margin-left: auto;margin-right: auto;width:98%;zoom:85%;">
                    <thead>
                        ...
                        if ($values['authlm'] = $l_initials) {
                            ...
                            <strong><i>Username: </i></strong><?php echo $row['current_user_id']; ?></br>
...
<div id="approveModal" class="reveal-modal" data-reveal aria-hidden="true" role="dialog">
  <p>You are about to approve <?php echo $row['names']; ?>'s request.</p>
  <?php
    echo "<form action='' method='post'>";
    echo "<input type='hidden' name='id' value= '" . $id . "' />";
    if ($row['authlevel'] == '0' && $row['a0'] === NULL) {
        echo '<button name="authlmpost" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #184818;">Approve</button></br>';
    }
    echo "</form>";
    ?>
  <a class="close-reveal-modal" aria-label="Close">&#215;</a>
</div>
<div id="declineModal" class="reveal-modal" data-reveal aria-hidden="true" role="dialog">
  <p>You are about to decline <?php echo $row['names']; ?>'s request. Please provide a reason below:</p>

  <?php
    echo "<form action='' method='post'>";
    echo "<input type='hidden' name='id' value= '" . $id . "' />";
    echo "<textarea rows='4' cols='50' name='decline_reason'></textarea>";
    if ($row['authlevel'] == '0' && $row['a0'] === NULL) {
      echo '<button name="authlmdecline" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #d41a22;">Decline</button>';
    }
    echo "</form>";
    ?>
                    ...
                    echo "</ul>";
                            ...
                            <td style="font-size:0.9rem;border-bottom: 1px solid black;vertical-align: text-top;">
                                ...
                                if ($row['authlevel'] == '1' && $row['a1'] === NULL) {
                                    echo '<button name="auth1post" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #184818;">Approve</button></br>';
                                    echo '<button name="auth1decline" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #d41a22;">Decline</button>';
                                }
                                echo "</form>";
                                ?>
                            </td>
...
";
                            ...
                            <td style="font-size:0.9rem;border-bottom: 1px solid black;vertical-align: text-top;">
                                ...
                                if ($row['authlevel'] == '2' && $row['a2'] === NULL) {
                                    echo '<button name="auth2post" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #184818;">Approve</button></br>';
                                    echo '<button name="auth2decline" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #d41a22;">Decline</button>';
                                }
                                echo "</form>";
                                ?>
                            </td>
...
";
                            ...
                            <td style="font-size:0.9rem;border-bottom: 1px solid black;vertical-align: text-top;">
                                ...
                                echo "<input type='hidden' name='id' value= '" . $id . "' />";
                                if ($row['authlevel'] == '3') {
                                    echo '<button name="auth3post" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #184818;">Approve</button></br>';
                                    echo '<button name="auth3decline" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #d41a22;">Decline</button>';
                                }
                                echo "</form>";
                                ?>
                            </td>
...
";
                            ...
                            <td style="font-size:0.9rem;border-bottom: 1px solid black;vertical-align: text-top;">
                                ...
                                echo "<input type='hidden' name='id' value= '" . $id . "' />";
                                if ($row['authlevel'] == '4') {
                                    echo '<button name="auth4post" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #184818;">Approve</button></br>';
                                    echo '<button name="auth4decline" type="submit" class="tiny round" style="font-size:0.9rem;border-bottom: 1px solid blackmargin:auto;padding:5px;background-color: #d41a22;">Decline</button>';
                                }
                                echo "</form>";
                                ?>
                            </td>
                ...
                <table  style="margin-left: auto;margin-right: auto;width:98%;zoom:85%;">
                    <thead>
                        <tr style="background-color:#00425f;border: 1px solid black;">
                            ...
                            $l_initials = $login_values['initials'];
...
if (isset($_POST['authlmpost'])) {
    ...
    $notify_auth1sql = mysqli_query($conn, "SELECT DISTINCT network.id as nid, initials, userid, email, authlm, auth1, auth2, auth3, auth4 FROM users, network 
WHERE network.id = '" . $id . "' AND auth1 = initials");
    $notify_values = mysqli_fetch_assoc($notify_auth1sql);


    $message = 'Hi,<br />
<br />
An authorization request is pending your approal. Click <a href="http://localhost/authforms/index.php">HERE</a> to view.';
    ...
    $subject1 = $mail->Subject = "AUTHORIZATION FORM REQUEST";      // Subject (which isn't required)  
    ...
    $mail->Password   = "Password123"; 
    // Compose
    $mail->SetFrom('frontdesk.management@interswitchgroup.com', 'Frontdesk');
    $subject1 = $mail->Subject = "YOU HAVE A VISITOR";      // Subject (which isn't required)  
    $mail->MsgHTML($message);
    ...
    // Send To  
    $mail->AddAddress($notify_values['email'], "Auth1 Name"); // Where to send it - Recipient
...
Your authorization request was declined by ' . $notify_reqvalues['uname'] . '. Reason: ' . $decline_reason . '.';
    ...
    // Authentication  
    $sender1 = $mail->Username = "centymjanja@gmail.com"; // Your full Gmail address
    $mail->Password = "Centy @ 91"; // Your Gmail password
    // Compose
    $mail->SetFrom("centymjanja@gmail.com", 'centy');
    $subject1 = $mail->Subject = "DECLINED AUTH. FORM REQUEST";      // Subject (which isn't required)  
    $mail->MsgHTML($message);

    // Send To  
    $mail->AddAddress($notify_reqvalues['nemails'], "Auth1 Name"); // Where to send it - Recipient
    $result = $mail->Send();    // Send!  
    //$message = $result ? 'Successfully Sent!' : 'Sending Failed!';      
    unset($mail);
...
An authorization request is pending your approal. Click <a href="http://localhost/authforms/index.php">HERE</a> to view.';
    ...
    // Compose
    $mail->SetFrom("centymjanja@gmail.com", 'centy');
    $subject1 = $mail->Subject = "AUTH. FORM REQUEST";      // Subject (which isn't required)  
    $mail->MsgHTML($message);
